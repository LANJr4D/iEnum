...
{
	return	([ModalAlert queryWith:question button1: @"Ja" button2: @"Nee"] == 0);
}

+ (BOOL) confirm: (NSString *) statement
{
	return	[ModalAlert queryWith:statement button1: @"Annuleer" button2: @"OK"];
}



+(void) alertWith:(NSString *)title andMessage:(NSString *)message
{
	CFRunLoopRef currentLoop = CFRunLoopGetCurrent();
	
	// Create Alert
	ModalAlertDelegate *madelegate = [[ModalAlertDelegate alloc] initWithRunLoop:currentLoop];
	UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title message:message delegate:madelegate cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
	[alertView show];
